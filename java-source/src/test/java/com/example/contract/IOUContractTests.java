...
public class IOUContractTests {
    ...
    public void transactionMustIncludeCreateCommand() {
        ...
        ledger(ledgerDSL -> {
            ledgerDSL.transaction(txDSL -> {
                txDSL.output(new IOUState(iou, miniCorp, megaCorp, new UniqueIdentifier()));
                txDSL.fails();
    ...
    public void transactionMustHaveNoInputs() {
        ...
        ledger(ledgerDSL -> {
            ledgerDSL.transaction(txDSL -> {
                txDSL.input(new IOUState(iou, miniCorp, megaCorp, new UniqueIdentifier()));
                txDSL.output(new IOUState(iou, miniCorp, megaCorp, new UniqueIdentifier()));
                txDSL.command(keys, IOUContract.Commands.Create::new);
    ...
    public void senderIsNotRecipient() {
        ...
        ledger(ledgerDSL -> {
            ledgerDSL.transaction(txDSL -> {
                txDSL.output(new IOUState(iou, megaCorp, megaCorp, new UniqueIdentifier()));
                PublicKey[] keys = new PublicKey[1];
                keys[0] = getMEGA_CORP_PUBKEY();
                txDSL.command(keys, IOUContract.Commands.Create::new);
